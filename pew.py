...
with tab1:
    ...
    with col2:
        today_feedings = len([f for f in st.session_state.feeding_history if f["timestamp"].date() == datetime.date.today()])
...
with tab2:
    ...
    with col1:
        portion_size = st.slider("Portion Size (cups)", min_value=0.25, max_value=5.0, value=1.0, step=0.25)
...
with tab3:
    ...
    with st.expander("➕ Add New Schedule", expanded=False):
        ...
        with col1:
            ...
            schedule_portion = st.number_input("Portion Size (cups)", min_value=0.25, max_value=5.0, value=1.0, step=0.25)
        with col2:
...
# Footer
st.divider()
st.caption(f"Smart Pet Feeder Control Panel | Last updated: {datetime.datetime.now()}")
