...
# ─────────────────────────────────────────────────────────
#  GLOBAL STATE — lives at module level, shared across ALL
#  Streamlit reruns and the Flask thread
# ─────────────────────────────────────────────────────────
import sys
_MODULE = sys.modules[__name__]

def _init_global():
    """Only initialise once per process, not on every Streamlit rerun."""
    if not hasattr(_MODULE, "_state"):
        _MODULE._lock  = threading.Lock()
        _MODULE._state = {
        ...
        _MODULE._flask_started = False

_init_global()

def get_state():
    with _MODULE._lock:
        return dict(_MODULE._state)

def update_state(**kwargs):
    with _MODULE._lock:
        _MODULE._state.update(kwargs)

def set_cmd(cmd):
    with _MODULE._lock:
        _MODULE._state["pending_cmd"] = cmd

def pop_cmd():
    with _MODULE._lock:
        cmd = _MODULE._state["pending_cmd"]
        _MODULE._state["pending_cmd"] = None
        return cmd
...
# Start Flask once per process
if not _MODULE._flask_started:
    threading.Thread(target=run_flask, daemon=True).start()
    _MODULE._flask_started = True
    time.sleep(0.5)
